...
# =============================================================================
# Step #2: Page config + font knobs (ONLY size tweaks)
# =============================================================================
...
FS_BADGE   = 12
FS_RECENT  = 12
INPUT_H    = max(24, int(FS_INPUT * 1.5))
...
INPUT_BG     = "#ffffff"
INPUT_BORDER = "#e6e9f2"
LEFT_BG      = "#e0e4ec"   # grey panel (unchanged)

# =============================================================================
# Step #2.1: Global UI CSS (layout only)
# =============================================================================
css(f"""
<style>
  /* Keep header always visible, compact overall page */
  .block-container {{ padding-top: 0.5rem; }}
  .page-header {{
    position: sticky; top: 0; z-index: 999;
    background: #fff; padding: 6px 0 4px 0; margin: 0 0 6px 0;
    display:flex; align-items:center; justify-content:space-between; gap: 8px;
    border-bottom: 1px solid #f0f0f0;
  }}
  .page-header__title {{
    font-size:{FS_TITLE}px; font-weight:800; margin:0; line-height:1.15; color:#111;
  }}
  .page-header__right img {{ height:30px; width:auto; display:block; }}

  .section-header {{ font-size:{FS_SECTION}px !important; font-weight:700; margin:.2rem 0 .35rem; }}

  .stNumberInput label, .stSelectbox label {{ font-size:{FS_LABEL}px !important; font-weight:700; }}
  .stNumberInput label .katex, .stSelectbox label .katex {{ font-size:{FS_LABEL}px !important; line-height:1.05 !important; }}
  ...
  div[data-testid="stNumberInput"] input[type="text"] {{
      ...
      line-height:{INPUT_H - 4}px !important; font-weight:600 !important; padding:4px 8px !important;
  }}
  div[data-testid="stNumberInput"] [data-baseweb*="input"] {{
      background:{INPUT_BG} !important; border:1px solid {INPUT_BORDER} !important;
      border-radius:8px !important; box-shadow:0 1px 2px rgba(16,24,40,.06) !important;
  }}
  div[data-testid="stNumberInput"] button {{ background:#fff !important; border:1px solid {INPUT_BORDER} !important; border-radius:6px !important; }}
  div[data-testid="stNumberInput"] {{ margin-bottom: 4px !important; }}

  /* Banner */
  .form-banner {{
    text-align:center; background: linear-gradient(90deg, #0E9F6E, #84CC16); color: #fff;
    padding:.3rem .5rem; border-radius:10px; font-weight:800; font-size:{FS_SECTION + 2}px; margin:.15rem 0 .5rem 0 !important;
  }}

  .prediction-result {{
    font-size:{FS_BADGE}px !important; font-weight:700; color:#2e86ab;
    background:#f1f3f4; padding:.35rem; border-radius:6px; text-align:center; margin-top:.35rem;
  }}

  .left-panel{{ background:{LEFT_BG} !important; border-radius:12px !important; box-shadow:0 1px 3px rgba(0,0,0,.1) !important; padding:10px !important; }}

  /* Tooltips */
  [data-baseweb="popover"], [data-baseweb="tooltip"], [data-baseweb="popover"] > div, [data-baseweb="tooltip"] > div {{
    background:#000 !important; color:#fff !important; border-radius:8px !important; padding:6px 8px !important; font-size:12px !important;
  }}

  /* ======== ACTION BAR (select + three buttons tightly grouped) ======== */
  .action-bar {{
    display:inline-flex; align-items:center; gap:8px; flex-wrap:nowrap;
    max-width: 720px; width: 100%;
  }}
  /* compact model select */
  .action-bar .model-select-wrap {{ width: 220px; min-width:220px; }}
  .action-bar .model-select-wrap > div > div {{ width:100% !important; }}
  .action-bar div[data-testid="stSelectbox"] > div > div {{ height:32px !important; }}

  .action-bar div.stButton > button {{
    background:#4CAF50 !important; color:#fff !important; font-weight:700 !important;
    font-size:{FS_BUTTON}px !important; border:none !important; border-radius:8px !important;
    height:32px !important; padding:0 10px !important;
  }}
  .action-bar div.stButton {{ margin:0 !important; padding:0 !important; }}
  .action-bar div.stButton > button:hover {{ filter:brightness(0.95); }}

  label[for="model_select_compact"] {{ font-size:{FS_SELECT}px !important; font-weight:700; margin-bottom:2px !important; }}
...
# =============================================================================
# Header (title + logo) — sticky + always tries to show a logo
# =============================================================================
def _logo_b64():
    for name in ["TJU logo.png", "logo2-01.png"]:
        p = (BASE_DIR / name)
        if p.exists():
...
st.markdown(
    ...
    <div class="page-header">
      ...
      <div class="page-header__right">
        {f'<img alt="Logo" src="data:image/png;base64,{_logo_b64()}" />' if _logo_b64() else ''}
      </div>
    </div>
...
# =============================================================================
# Step #4: Model loading (unchanged logic)
# =============================================================================
...
# Step #6: Right panel
# =============================================================================
HERO_W = 260  # small figure to fit everything
with right:
    try:
    ...
    # ACTION BAR — tightly grouped inline
    st.markdown('<div class="action-bar">', unsafe_allow_html=True)

    # tiny columns only to host widgets; the CSS keeps them tight
    col_sel, col_calc, col_reset, col_clear = st.columns([1, 0.28, 0.28, 0.33], gap="small")

    with col_sel:
        st.markdown("<div class='model-select-wrap'>", unsafe_allow_html=True)
...
def predict_di(choice, _unused_array, input_df):
    ...
    if choice == "LightGBM":
        mdl = model_registry["LightGBM"]; prediction = float(mdl.predict(X)[0])
    if choice == "XGBoost": prediction = float(model_registry["XGBoost"].predict(X)[0])
    if choice == "CatBoost": prediction = float(model_registry["CatBoost"].predict(X)[0])
    if choice == "Random Forest": prediction = float(model_registry["Random Forest"].predict(X)[0])
    if choice == "PS":
...
def render_di_chart(results_df: pd.DataFrame, curve_df: pd.DataFrame,
    ...
    text_layer = alt.Chart(curve).mark_text(align='left', dx=6, dy=-6, fontSize=12, fontWeight='bold', color='red').encode(
    ...
    chart_html = chart_html.replace('</style>',
        '</style><style>.vega-embed .vega-tooltip, .vega-embed .vega-tooltip * {{ font-size: 12px !important; background: #000 !important; color: #fff !important; padding: 8px !important; }}</style>')
    st.components.v1.html(chart_html, height=size + 70)
...
with right:
    render_di_chart(st.session_state.results_df, _curve_df, theta_max=THETA_MAX, di_max=1.5, size=300)
...
if show_recent and not st.session_state.results_df.empty:
    ...
    for i, row in st.session_state.results_df.tail(5).reset_index(drop=True).iterrows():
        st.markdown(
            f"<div style='background:#f8f9fa; padding:.35rem; margin:.2rem 0; border-radius:5px; border-left:4px solid #4CAF50; font-weight:600; display:inline-block;'>"
